...
public:
	...
	size_t nthreads = 1;
	std::string outfname = "";
	uint64_t randseed = 0x3355557799AACCULL;
...
int Sketchargs::usage(const int argc, const char *const *argv) {
	...
	std::cerr << "  --isstrandpreserved : Preserve strand, which means ignore reverse complement. [" << isstrandpreserved << "]\n\n";
	std::cerr << "  --iscasepreserved : Preserve case, which means the lowercase and uppercase versions of the\n" 
	           << "     same letter are treated as two different letters. [" << iscasepreserved << "]\n\n";
	std::cerr << "  --randseed : Seed to provide to the hash function. [" << randseed << "]." << "\n\n";
	std::cerr << "  --outfname : Name of the file containing sketches as output [" << outfname << "]." << "\n\n";
	exit(1);
}
...
int Sketchargs::_parse(std::string arg) {
	...
	else if ("--iscasepreserved" == key) { issval >> iscasepreserved; }
	else if ("--isstrandpreserved" == key) { issval >> isstrandpreserved; }
...
int Distargs::usage(const int argc, const char *const *argv) {
	...
	std::cerr << "  --nneighbors : only this number of best-hit results per query are reported [" << nneighbors << "]\n\n";
	std::cerr << "  --nthreads : This many threads will be spawned for processing. [" << nthreads << "]\n\n";
	std::cerr << "  --outfname : Name of the file containing comparison between query and target.\n"
	          << "    This file name ends with the suffix " << suffix << " (not applicable to stdout).\n"
	...
	std::cerr << "Note:\n\n";
	std::cerr << "  If target-sketch is omitted and --nneighbors is greater than the total number of genomes,\n" 
